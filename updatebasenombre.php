...
if (!empty($_POST)) {
    ...
    if ($valid) {
        ...
        $sql = "UPDATE basenombre SET basenombreid = ?, basenombre = ? WHERE basenombreid = ?;";
        $q = $pdo->prepare($sql);
    ...
    $pdo = Database::connect();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql = "SELECT * FROM basenombre WHERE basenombreid = ?;";

    $q = $pdo->prepare($sql);
    $q->execute(array($id));
    $data = $q->fetch(PDO::FETCH_ASSOC);
    $f_id = $data['basenombreid'];
    $nombrebase = $data['basenombre'];
    Database::disconnect();
}
...
<body>
    <div class="container">
        <div class="span10 offset1">
            ...
            <form action="updatebasenombre.php?id=<?php echo $id ?>" class="form-horizontal" method="post">
                ...
                <div class="control-group <?php echo !empty($nombrebaseError) ? 'error' : ''; ?>">
                    ...
                    <div class="controls">
                        <input name="nombrebase" placeholder="Tipo Modelo" type="text" value="<?php echo !empty($nombrebase) ? $nombrebase : ''; ?>">
