...
if (!empty($_POST)) {
    ...
    if ($valid) {
        ...
        $sql = "UPDATE inventariosalas SET salaid = ?, salanombreid = ?, maderaid = ?, telaid = ?, colorid = ?, preciosala = ? WHERE salaid = ?;";
        $q = $pdo->prepare($sql);
        $q->execute(filter_var_array(array($f_id, $salanombre, $madera, $tela, $color, $preciosala, $id), FILTER_SANITIZE_STRING));
        Database::disconnect();
        header("Location: innovosalas.php");
    }
} else {
    $pdo = Database::connect();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $sql = "SELECT * FROM inventariosalas WHERE salaid = ?;";

    $q = $pdo->prepare($sql);
    $q->execute(array($id));
    $data = $q->fetch(PDO::FETCH_ASSOC);
    $f_id = $data['salaid'];
    $salanombre = $data['salanombreid'];
    $madera = $data['maderaid'];
    $tela = $data['telaid'];
    $color = $data['colorid'];
    $preciosala = $data['preciosala'];
    Database::disconnect();
}
...
<body>
    <div class="container">
        <div class="span10 offset1">
            ...
            <form action="updatesala.php?id=<?php echo $id ?>" class="form-horizontal" method="post">
